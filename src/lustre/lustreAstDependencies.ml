...
type contract_summary = (LA.ident list) NodeId.Map.t
...
let empty_node_summary: node_summary = NodeId.Map.empty

let empty_contract_summary: contract_summary = NodeId.Map.empty
...
let mk_graph_contract_decl: Lib.position -> LA.contract_node_decl -> dependency_analysis_data
  = fun pos (node_id, _, ips, ops, (_, c)) ->
  let i = (HString.concat2 contract_prefix (NI.get_internal_name node_id)) in
...
let mk_graph_node_decl: Lib.position -> LA.node_decl -> dependency_analysis_data
  ...
  let cg = connect_g_pos
             (match contract_opt with
              ...
              | Some (_, c) -> List.fold_left union_dependency_analysis_data empty_dependency_analysis_data
                            (List.map (mk_graph_contract_node_eqn (NI.get_internal_name node_id)) c))
             (HString.concat2 node_prefix (NI.get_internal_name node_id)) pos in
  ...
  List.fold_left
    (fun g (nr, p) -> union_dependency_analysis_data g
                        (connect_g_pos
                           (singleton_dependency_analysis_data node_prefix nr p) (NI.get_internal_name node_id) pos))
    cg node_refs
...
let rec  mk_decl_map: LA.declaration option IMap.t -> LA.t -> ((LA.declaration option IMap.t), [> error]) result =
  ...
  | LA.ContractNodeDecl (span, (node_id, _, _, _, _)) as cndecl :: decls ->
    let {LA.start_pos = pos} = span in
    let* m' = check_and_add m pos contract_prefix (NI.get_internal_name node_id) (Some cndecl) in
    mk_decl_map m' decls
...
let rec mk_contract_eqn_map: LA.contract_node_equation option IMap.t -> LA.contract_node_equation list -> ((LA.contract_node_equation option IMap.t), [> error]) result
  ...
  | (LA.GhostVars (_, (GhostVarDec(_, [])), _)) :: eqns -> 
    mk_contract_eqn_map m eqns
  | (LA.ContractCall (pos, node_id, _, _, _) as cc ) :: eqns -> 
    let* m' = check_and_add m pos contract_prefix (NI.get_internal_name node_id) (Some cc) in
    mk_contract_eqn_map m' eqns  
...
let sort_and_check_contract_eqns: dependency_analysis_data
  ...
  Debug.parse "Sorting contract equations for %a" NI.pp_print_node_id_user_name i;
...
let rec generate_summaries: dependency_analysis_data -> LA.t -> dependency_analysis_data
  ...
  | LA.ContractNodeDecl (_, cdecl) :: decls ->
     ...
     generate_summaries {ad with csummary = NodeId.Map.union (fun _ _ v2 -> Some v2) ad.csummary cs } decls
