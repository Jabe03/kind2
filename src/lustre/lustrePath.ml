...
let pp_print_mixed_section_pt ident_width val_width sect ppf (streams, adt_streams, provided_inputs) =
  ...
  | _ ->
    ...
    (match provided_inputs with [] -> ()
    | _ -> Format.fprintf ppf "%a@," (pp_print_list (pp_print_string_valued_stream_pt ident_width val_width) "@,") provided_inputs)
    

  (* Output sequences of values for each stream of the nodes in the list
   and for all its called nodes *)
let rec pp_print_lustre_path_pt' ?(full_contract=false) is_top const_map const_funcs globals provided_inputs ppf = function
...
| (
  ...
  if NI.get_node_type node_id = FreeConstant || NI.get_node_type node_id = TypeAscription || NI.get_node_type node_id = ClockedExpr then
    pp_print_lustre_path_pt' false const_map const_funcs globals provided_inputs ppf tl
  else 
  ...
  Format.fprintf ppf "@[<v>\
    ...
    (pp_print_mixed_section_pt ident_width val_width "Inputs") (inputs', adt_inputs, provided_inputs)
...
(* Output sequences of values for each stream of the node and for all
   its called nodes *)
let pp_print_lustre_path_pt ?(full_contract = false) globals provided_inputs ppf (lustre_path, const_map) =
  ...
  pp_print_lustre_path_pt' ~full_contract true const_map const_funcs globals provided_inputs ppf [lustre_path]
...
let pp_print_path_pt
  ...
  |> pp_print_lustre_path_pt ~full_contract:full_contract globals provided_inputs ppf
...
let pp_print_streams_list_json (provided_inputs:(HString.t * string list) list) set_map_outputs provided_types node model clock adt_tagged ppf streams = 
    ...
    let all_printers =
      provided_printers @ stream_printers @ set_map_output_printers @ adt_tagged_printers
    in
...
let pp_print_streams_json (provided_inputs:(HString.t * string list) list) provided_types is_top const_map const_funcs globals ppf
  ...
  Format.fprintf ppf "%a"
    (pp_print_streams_list_json set_map_inputs set_map_outputs provided_types node model clock adt_tagged) streams
...
(* Output a list of node models. *)
let rec pp_print_lustre_path_json' is_top const_map const_funcs globals provided_inputs provided_types ppf = function
  ...
  | (
    ...
    (* Functions derived from constants are printed along with the global constants. 
       Type ascriptions not shown. *)
    if NI.get_node_type node_id = FreeConstant || NI.get_node_type node_id = TypeAscription || NI.get_node_type node_id = ClockedExpr then
      pp_print_lustre_path_json' false const_map const_funcs globals provided_inputs provided_types ppf tl
    else
    ...
    let pp_print_subnodes_json ppf = function
      ...
      | subnodes ->
          Format.fprintf ppf ",@,\"subnodes\" :@,[@[<v 1>%a@]@,]"
            (pp_print_lustre_path_json' false const_map const_funcs globals provided_inputs provided_types) subnodes
    in
    ...
    (* Format.fprintf ppf
       ...
       (pp_print_streams_json provided_inputs provided_types is_top const_map const_funcs globals) (node, model, call_conds)
...
(* Output sequences of values for each stream of the node and for all
   its called nodes *)
let pp_print_lustre_path_json globals (provided_inputs: (HString.t * string list) list) (provided_types:  LustreAst.lustre_type HString.HStringMap.t) ppf (path, const_map) =
  let const_funcs = get_const_func_info (snd path) in
  let const_funcs = process_const_funcs const_funcs path in

  (* Delegate to recursive function *)
  Format.fprintf ppf "@,[@[<v 1>%a@]@,]"
    (pp_print_lustre_path_json' true const_map const_funcs globals provided_inputs provided_types) [path]
...
=
  ...
  node_path_of_subsystems
    globals first_is_init trans_sys model subsystems
  (* Output as JSON *)
  |> pp_print_lustre_path_json globals provided_inputs provided_types ppf



let rec pp_print_lustre_path_json_testgen' const_map const_funcs provided_inputs provided_types ppf = function
  | [] -> ()
  | (
    _, Node (node,
    ...
    (* Continue *)
    pp_print_lustre_path_json_testgen' const_map const_funcs provided_inputs provided_types ppf tl
